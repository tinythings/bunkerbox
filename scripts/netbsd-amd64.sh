...
require_linker() {
	command -v cc >/dev/null 2>&1 || command -v clang >/dev/null 2>&1 || command -v gcc >/dev/null 2>&1 || {
		echo "NetBSD worker setup requires a C compiler (cc, clang, or gcc)." >&2
...
setup() {
	[ -n "${HOME:-}" ] || {
		echo "NetBSD worker setup requires HOME for a user-local Rust toolchain" >&2
		exit 1
	}
	...
	if [ "$(id -u)" -eq 0 ]; then
		echo "NetBSD worker setup needs Rust, but this process is root." >&2
	...
	if ! command -v rustup >/dev/null 2>&1; then
		...
		if command -v curl >/dev/null 2>&1; then
			...
			echo "NetBSD worker setup requires curl, fetch, or wget." >&2
			echo "Install one as root, then rerun the worker build." >&2
			exit 1
		fi

		if ! sh "$rustup_script" -y --profile minimal --default-toolchain stable --no-modify-path; then
			echo "NetBSD worker setup could not install Rust user-locally." >&2
	...
	if ! rustup toolchain install stable --profile minimal; then
		echo "NetBSD worker setup could not install the stable Rust toolchain." >&2
		exit 1
	fi
	if ! rustup default stable; then
		echo "NetBSD worker setup could not select the stable Rust toolchain." >&2
		exit 1
	fi

	command -v "$CARGO" >/dev/null 2>&1 && command -v rustc >/dev/null 2>&1 || {
		echo "NetBSD worker setup could not provide cargo and rustc." >&2
		exit 1
	}
	require_linker
}
...
case "$ACTION" in
	...
	*)
		echo "Unsupported NetBSD worker action: $ACTION" >&2
		exit 1 ;;
esac
