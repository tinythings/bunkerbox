...
pub fn cli(version: &'static str) -> Command {
    ...
    Command::new(APPNAME)
        ...
        .about(format!(
            ...
            "run AI agents inside a Kata bunker"
        ))
        .override_usage(format!("{APPNAME} <COMMAND>"))
        .subcommand(
            Command::new("setup")
                .about("IT: setup host runtime")
                .styles(styles.clone())
                .disable_help_flag(true)
                .arg(help_arg()),
        )
        .subcommand(
            Command::new("install-image")
                .about("IT: import prepared OCI image")
                .styles(styles.clone())
                .disable_help_flag(true)
                .arg(help_arg()),
        )
        ...
        .subcommand(
            Command::new("list")
                .about("List embedded YAML sequences")
                .styles(styles.clone())
                .disable_help_flag(true)
                .arg(help_arg()),
        )
        ...
        .styles(styles)
        .after_help(
            "NOTE: bunkerbox is a proof of concept.\n"
                .bright_yellow()
                .to_string(),
        )
}
