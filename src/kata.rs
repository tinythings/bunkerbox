...
pub fn run(
    config: &RuntimeConfig, workspace_mode: WorkspaceMode, quota: u64, runtime_exclude: Option<&[String]>, container_name: &str, _share_dir: &Path,
    app_name: &str,
) -> Result<(), String> {
...
fn setup_session(home_path: &Path, session_mb: u32, _container_name: &str, uid: &str, gid: &str) -> Result<PathBuf, String> {
    let bunker_dir = home_path.join(".bunker");
    let session_img = bunker_dir.join("session.img");
    let session_dir = bunker_dir.join("session");
