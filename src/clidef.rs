...
pub fn cli(version: &'static str) -> Command {
    ...
    Command::new(APPNAME)
        ...
        .subcommand(
            Command::new("sync")
                .about("Copy overlay changes back to the real repository")
                .styles(styles.clone())
                .disable_help_flag(true)
                .arg(help_arg()),
        )
        ...
        .arg(Arg::new("log").long("log").value_name("PATH").help("Write diagnostics and proxied command output to the given file"))
        .arg(help_arg())
